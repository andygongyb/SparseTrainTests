...
void simple_net(int sparsity, int n, int mb, int ic, int ih, int iw, int oc, int oh, int ow, int kh, int kw)
{
    ...
    auto conv_desc = convolution_forward::desc(
            prop_kind::forward_training, convolution_winograd, c_src_desc_f,
    ...
    auto conv_bwd_data_desc = convolution_backward_data::desc(
            convolution_winograd, c_src_desc, c_weights_desc, c_dst_desc,
            { strh, strw }, { dilh, dilw },
            { padh, padw }, padR, padding_kind::zero);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        simple_net(sparsity, n, mb, ic, ih, iw, oc, oh, ow, kh, kw);
